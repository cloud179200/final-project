...
interface Props {
  ...
  url: string;
  initialFilter?: IUserFilter;
}

const UserDataTable = (props: Props) => {
  const dispatch = useDispatch<ThunkDispatch<AppState, null, Action<string>>>();
  const { users, selectedUsers, setSelectedUsers, setFilterByPage, url, initialFilter } = props;
  const { pageInfoUser } = useSelector((state: AppState) => ({
    pageInfoUser: state.user.pageInfoUser,
  }));
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(25);
  const [sortBy, setSortBy] = useState<{ sort: string; order_by: 'ASC' | 'DESC' }>({
    sort: 'last_login',
    order_by: 'DESC',
  });
  ...
  useEffect(() => {
    ...
    setPage(pageInfoUser.index);
    setRowsPerPage(pageInfoUser.count);
      ...
      >
        <TableHead>
          <TableRow>
            ...
            <StyledTableCell align="left" sx={{ cursor: 'pointer' }} onClick={(e) => handleSwitchSortBy('fistName')}>
            ...
            <StyledTableCell align="left" sx={{ cursor: 'pointer' }} onClick={(e) => handleSwitchSortBy('created')}>
              Created&nbsp;{getSortIcon('created')}
            </StyledTableCell>
            <StyledTableCell align="left" sx={{ cursor: 'pointer' }} onClick={(e) => handleSwitchSortBy('last_login')}>
        ...
        <TableBody>
          {users &&
            (rowsPerPage > 0 ? users.detail.slice(0, rowsPerPage) : users.detail).map((user) => (
              <UserDataTableRow
                url={url}
                key={user.profile_id}
                user={user}
