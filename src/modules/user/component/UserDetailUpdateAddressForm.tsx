...
import { validateUpdateAddress, validUpdateAddress } from "../utils";
...
interface Props {
    ...
    onUpdateAddress: (values: IInfoUserDetailAddressBook) => void;
    addressToUpdate: IInfoUserDetailAddressBook;
}
const UserDetailUpdateAddressForm = (props: Props) => {
    const dispatch = useDispatch<ThunkDispatch<AppState, null, Action<string>>>()
    const { countries, states } = useSelector((state: AppState) => ({
        countries: state.data.countries,
        states: state.data.states
    }));
    const { getStates, onUpdateAddress, addressToUpdate } = props
    ...
    const submit = useCallback((e: any) => {
        e.preventDefault();
        const validateValues = validateUpdateAddress(formValues);
        console.log(validateValues)
        setValidate(validateValues);

        if (!validUpdateAddress(validateValues)) {
            return;
        }
        onUpdateAddress(formValues)
    }, [formValues, onUpdateAddress])
    useEffect(() => {
        const validateValues = validateUpdateAddress(formValues);
    ...
    return <Container maxWidth="sm" component="form" sx={{ display: "flex", justifyContent: "center", alignItems: "center", height: "100vh" }} onSubmit={submit}>
        <Grid
            ...
            <Grid item xs={12} p={2} sx={{ justifyContent: "flex-start", alignItems: "center", borderBottom: "1px solid #1b1b38" }}>
                <Button disabled={!validUpdateAddress(validate)} color="secondary" variant="contained" type="submit">Save changes</Button>
            </Grid>
        </Grid>
    </Container>
}
export default UserDetailUpdateAddressForm