...
import { setBrands, setCategories, setConditions, setLoadingProductData, setProducts, setShippings } from "../redux/productReducer"
interface Props {
    url: string;
}
const ProductPage = (props: Props) => {
    ...
    const { user, products, pageInfoProduct } = useSelector((state: AppState) => ({
    ...
    const [modal, setModal] = useState({ openConfirmDelete: false, openExportCSV: false })
    ...
    const deleteProducts = useCallback(async () => {
        if (selectedProducts.length < 1 || !user) {
            return
        }
        dispatch(setLoadingProductData(true))
        const productsForDelete = [...selectedProducts].filter(detail => detail.id !== user.profile_id)
        const paramsForDelete = productsForDelete.map((item) => { return { id: item.id, delete: 1 } })
    ...
    const getProducts = useCallback(async () => {
        ...
        dispatch(setLoadingProductData(true))
        const json = await dispatch(
            fetchThunk(API_PATHS.productsList, "post", { ...filter, page: pageInfoProduct.index+1, count: pageInfoProduct.count })
        );
        dispatch(setLoadingProductData(false))
        if (!json?.errors) {
            if (!json.data) {
            ...
            dispatch(setProducts({ detail: [...json.data], recordsTotal: json.recordsTotal, recordsFiltered: json.recordsFiltered }))
            return;
        }
    ...
    const getExportFile = useCallback(async (fileLink: string) => {
        dispatch(setLoadingProductData(true))
        const json = await dispatch(
            fetchThunk(API_PATHS.exportDownload, "post", { file: fileLink })
        );
        dispatch(setLoadingProductData(false))
        if (!json?.errors) {
            const url = json.data
    ...
    const exportCSV = useCallback(async () => {
        ...
        dispatch(setLoadingProductData(true))
        const json = await dispatch(
            fetchThunk(API_PATHS.productsExport, "post", id.length > 0 ? { id } : {})
        );
        dispatch(setLoadingProductData(false))
        if (!json?.errors) {
    ...
    return <Box component="div" sx={{
        ...
        {products && <>
            <Typography variant="h4" pb={2} sx={{ color: "#fff" }}>Products</Typography>
            <Box component="div" pb={2}>
                <ProductFilter setFilterByPage={handleSetFilterForProductFilter} />
            </Box>
            <Box component="div" pb={4} pt={4}>
                <Button color="secondary" variant="contained" onClick={handleAddProductClick}>Add Product</Button>
            </Box>
            <Box component="div" sx={{ overflow: "auto", minWidth: "100%" }}>
                <ProductDataTable url={url} products={products} setFilterByPage={handleSetFilterForProductDataTable} selectedProducts={selectedProducts} setSelectedProducts={(selected: Array<IProduct>) => { setSelectedProducts([...selected]) }} />
            </Box>
            <Box component="div" mt={2} p={2} width={1} sx={{ backgroundColor: "#323259", position: "sticky", border: "1px solid #1b1b38", borderWidth: "0 0 1px 1px", boxShadow: "0 0 13px 0 #b18aff", bottom: "0" }}>
                <Button disabled={selectedProducts.length === 0} color="warning" variant="contained" onClick={(e) => setModal({ ...modal, openConfirmDelete: true })}>Remove selected</Button>
                <Button color="warning" sx={{ marginLeft: 2 }} variant="contained" onClick={(e) => setModal({ ...modal, openExportCSV: true })}>{selectedProducts.length < 1 ? "Export all: CSV" : "Export selected: CSV"}</Button>
            </Box>
            <Modal
            ...
            >
                <Container maxWidth="xs" sx={{ display: "flex", justifyContent: "center", alignItems: "center", height: "100vh" }}>
                    <Grid
                        ...
                        <Grid item xs={12} pb={1} p={2} sx={{ justifyContent: "flex-start", alignItems: "center", borderBottom: "1px solid #1b1b38" }}>
                            <Typography color="#fff" variant="body1" fontSize=".9375rem" fontWeight="bold">Confirm Export</Typography>
                        </Grid>
                        <Grid item xs={12} p={2} sx={{ justifyContent: "flex-start", alignItems: "center", borderBottom: "1px solid #1b1b38" }}>
                            <Typography color="#fff" variant="body1" fontSize=".9375rem">Do you want to export all products ?</Typography>
                        </Grid>
                        <Box component="span" width={1} p={2} sx={{ display: "flex", flexDirection: "column", '& .MuiTextField-root': { width: '100%', pb: 2 }, '& .MuiFormControl-root': { pb: 2 }, "& .MuiAlert-root": { mb: 2 } }}>
                            <Box width={1} display="flex" justifyContent="space-between" alignItems="center">
                                <Button variant="contained" color="secondary" onClick={handleConfirmExportCSVCLick}>Yes</Button>
                                <Button variant="contained" color="error" onClick={(e) => setModal({ ...modal, openExportCSV: false })}>No</Button>
