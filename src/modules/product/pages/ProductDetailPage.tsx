...
import { setBrands, setCategories, setConditions, setLoadingProductData, setProductDetail, setShippings, setVendors } from "../redux/productReducer";
...
const ProductDetailPage = (props: Props) => {
    ...
    const { productDetail, vendors, categories, conditions, brands, shippings } = useSelector((state: AppState) => ({
    ...
    const onUpdateProduct = useCallback(async (values: IUpdateProductParams, files: Array<File>) => {
        dispatch(setLoadingProductData(true))
        const formData = new FormData()
                ...
                <Grid container width={1}>
                    ...
                    <Grid item xs={12}>
                        <TabPanel value={tabIndex} index={0}>
                            {(productDetail && vendors && categories && conditions && brands && shippings && true) && <ProductDetailUpdateForm onUpdateProduct={handleUpdateProduct} productDetail={productDetail} vendors={vendors} categories={categories} conditions={conditions} brands={brands} shippings={shippings} />}
                        </TabPanel>
