...
import { IProductDetail, IUpdateProductParams } from '../../../models/product';
...
const ProductDetailUpdateForm = (props: Props) => {
  ...
  const removeImage = (idImage: string) => {
    ...
    const newImageOrder = [...formik.values.imagesOrder].filter((item) => item !== image.file);
  ...
  const handleAddShippingZone = (id?: number) => {
    ...
    const isShippingZoneIn = [...formik.values.shipping_to_zones].find((item) => item.id === id);
    if (isShippingZoneIn) {
      return;
    }
    formik.setValues({
      ...formik.values,
      shipping_to_zones: [...formik.values.shipping_to_zones, { id, price: '0.00' }],
    });
  };
  ...
  useEffect(() => {
    const newImageOrder = [
      ...productDetail.images.filter((item) => !isImageDeleted(item.id)).map((item) => item.file),
            ...
            <Grid container width={1} p={2} spacing={3}>
              ...
              <Grid item xs>
                <Autocomplete
                  ...
                  value={vendors?.find((item) => item.id === formik.values.vendor_id) || null}
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      required
            ...
            <Grid container width={1} p={2} spacing={3}>
              ...
              <Grid item xs>
                <Autocomplete
                  ...
                  value={brands?.find((item) => item.id === formik.values.brand_id) || null}
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      required
            ...
            <Grid container width={1} p={2} spacing={3}>
              ...
              <Grid item xs>
                <Autocomplete
                  ...
                  value={conditions?.find((item) => item.id === formik.values.condition_id) || null}
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      required
                  ...
                  {productDetail.images.map((item) => {
                    ...
                    return (
                      <Badge
                        ...
                        sx={{ mr: 2 }}
                        badgeContent={
                          <IconButton color="error" onClick={(e) => removeImage(item.id)}>
                            <RemoveCircleRounded />
                          </IconButton>
                        }
                      >
